...
export function errorHandler(err: Error, _req: Request, res: Response, _next: NextFunction): void {
  ...
  if (err instanceof SyntaxError && "status" in err && (err as { status?: number }).status === 400) {
    res.status(400).json({ error: "JSON malformado en la petición" });
    return;
  }
  ...
  if (mysqlErr.errno === 1451) {
    res.status(409).json({ error: "El registro tiene dependencias y no puede eliminarse" });
    return;
  }
  if (mysqlErr.errno === 1452) {
    res.status(400).json({ error: "Referencia foránea inválida" });
    return;
  }
